...
@section('content')
    <h1 class="text-base md:text-lg font-bold text-gray-800 mb-1">Comments</h1>
    <p class="text-sm text-gray-500 mb-3">Review and moderate visitor comments</p>

    <div class="flex flex-wrap gap-2 mb-3 text-sm">
        <a href="{{ url('/admin/comments') }}" class="px-3.5 py-1.5 rounded-lg transition-colors {{ !$status ? 'bg-sky-600 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">All ({{ $counts['total'] }})</a>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($comments as $comment)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                @if($comment->post)
                                    <a href="{{ url('/blog/' . $comment->post->slug) }}" class="text-sky-600 hover:underline text-xs" target="_blank">{{ Str::limit($comment->post->title, 30) }}</a>
                                @else
