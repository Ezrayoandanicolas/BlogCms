...
@section('content')
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-3">
        <div>
            <h1 class="text-base md:text-lg font-bold text-gray-800">Post Calendar</h1>
            ...
            @for($day = 1; $day <= $daysInMonth; $day++)
                ...
                <div class="min-h-[100px] p-2 border-b border-r border-gray-50 {{ $isToday ? 'bg-blue-50' : ($isPast ? 'bg-white' : 'bg-amber-50/30') }}">
                    <div class="flex items-center justify-between mb-1">
                        <span class="text-sm font-semibold {{ $isToday ? 'text-sky-600' : 'text-gray-700' }}">{{ $day }}</span>
                        @if($dayPosts->count() > 0)
                            <span class="text-[10px] font-medium text-white {{ $isFuture ? 'bg-amber-500' : 'bg-sky-500' }} px-1.5 py-0.5 rounded-full">{{ $dayPosts->count() }}</span>
                        @endif
                    </div>
