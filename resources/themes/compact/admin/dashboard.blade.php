...
@section('content')
    <!-- Stats Cards -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-3 mb-4">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-3">
                <div class="w-10 h-10 rounded-xl bg-blue-50 text-sky-600 flex items-center justify-center">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"/></svg>
                </div>
                <span class="text-xs font-medium text-sky-600 bg-blue-50 px-2 py-0.5 rounded-full">{{ $stats['published'] }} published</span>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
            ...
            <p class="text-3xl font-bold text-gray-800">{{ $postsThisWeek }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-4">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            <h3 class="font-semibold text-gray-800 mb-2">Posts — Last 30 Days</h3>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            <h3 class="font-semibold text-gray-800 mb-2">Per Domain</h3>
            <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <a href="{{ url('/admin/posts/create') }}" class="flex items-center gap-3 p-3 rounded-xl bg-blue-50 text-sky-700 hover:bg-sky-100 transition-colors text-sm font-medium">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            <h3 class="font-semibold text-gray-800 mb-2">Content Summary</h3>
            <div class="space-y-4">
                <div>
                    <div class="flex items-center justify-between text-sm mb-1">
                        <span class="text-gray-600">Published Posts</span>
                        <span class="font-semibold text-gray-800">{{ $stats['published'] }} / {{ $stats['posts'] }}</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-2">
                        @php $pct = $stats['posts'] > 0 ? round(($stats['published'] / $stats['posts']) * 100) : 0; @endphp
                        <div class="bg-sky-600 h-2 rounded-full transition-all" style="width: {{ $pct }}%"></div>
                    </div>
                </div>
                <div>
                    <div class="flex items-center justify-between text-sm mb-1">
                        <span class="text-gray-600">Active Comments</span>
                        <span class="font-semibold text-gray-800">{{ $counts['approved'] ?? 0 }}</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-2">
