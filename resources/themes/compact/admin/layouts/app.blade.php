    ...
    <div class="min-h-screen flex flex-col lg:flex-row">
        <aside class="lg:w-64 bg-gradient-to-b from-slate-800 to-slate-900 text-white shrink-0">
            <div class="p-5 border-b border-slate-700 flex items-center justify-between">
                <a href="{{ url('/admin') }}" class="text-sm font-bold tracking-tight">{{ config('app.name') }}</a>
            ...
            <div class="p-4 border-b border-slate-700">
                ...
                <form action="{{ url('/admin/switch-domain') }}" method="POST" id="domain-switch-form">
                    ...
                    <select name="domain_id" onchange="this.form.submit()"
                            class="w-full bg-slate-700 text-white border border-slate-600 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
        ...
        <main class="flex-1 flex flex-col min-w-0">
            <header class="bg-white border-b px-4 md:px-8 py-3 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3 text-sm text-gray-500">
                    ...
                    <span class="w-7 h-7 rounded-full bg-sky-600 text-white flex items-center justify-center text-xs font-bold">{{ strtoupper(substr(auth()->user()?->name ?? 'G', 0, 1)) }}</span>
                </div>
            </header>

            <div class="flex-1 p-4 md:p-8 overflow-x-auto">
                @if(session('success'))
                    <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-xl mb-3 text-sm flex items-center gap-2">
