...
<div x-data="apiDocs()" class="max-w-5xl">
    <h1 class="text-base md:text-lg font-bold text-gray-800 mb-1">API Documentation</h1>
    <p class="text-sm text-gray-500 mb-3">Test and explore the API endpoints</p>

    @if($newToken ?? session('newToken'))
        <div class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl mb-3 text-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" x-model="baseUrl" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none font-mono">
                </div>
                <div>
                    ...
                    <input type="text" x-model="bearerToken" placeholder="paste token here"
                           class="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none font-mono">
                </div>
            </div>
        </div>
    </div>

    <div class="mb-2 flex gap-2 flex-wrap border-b border-gray-200 pb-2">
        <template x-for="g in groups" :key="g.key">
            <button @click="activeGroup = g.key"
                    :class="activeGroup === g.key ? 'bg-sky-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
                                ...
                                <div class="space-y-2">
                                            ...
                                            <div>
                                                ...
                                                <template x-if="tp.type === 'file'">
                                                    <input type="file" :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name"
                                                           class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-sky-500 outline-none">
                                                </template>
                                                <template x-if="tp.type === 'textarea'">
                                                    <textarea :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name" rows="3"
                                                              class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-mono focus:ring-2 focus:ring-sky-500 outline-none"
                                                ...
                                                <template x-if="tp.type !== 'file' && tp.type !== 'textarea'">
                                                    <input :type="tp.type || 'text'" :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name"
                                                           class="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-sky-500 outline-none font-mono"
                                    ...
                                    <div class="flex items-center gap-3 mt-3">
                                        <button @click="testEndpoint(ep)"
                                                class="bg-sky-600 text-white px-4 py-1.5 rounded-lg text-xs font-medium hover:bg-sky-700 transition-colors">
                                            Send Request
                                        </button>
...
function apiDocs() {
    ...
    return {
        ...
        methodClass(m) {
            const map = { GET: 'bg-emerald-100 text-emerald-700', POST: 'bg-sky-100 text-sky-700', PUT: 'bg-amber-100 text-amber-700', DELETE: 'bg-red-100 text-red-700' };
