...
@section('content')
    <h1 class="text-base md:text-lg font-bold text-gray-800 mb-1">Settings</h1>
    <p class="text-sm text-gray-500 mb-3">Site configuration per domain</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-4">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            ...
            @if($editingDomain)
                <p class="text-xs text-gray-400 mb-2">Editing: <span class="font-medium text-gray-600">{{ $editingDomain->domain }}</span></p>
            @endif
            <form action="{{ route('admin.settings.update') }}" method="POST">
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="site_name" value="{{ $siteSettings['site_name'] }}" required
                           class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-shadow">
                </div>
                <div class="mb-5">
                    <label class="block text-xs font-medium text-gray-500 mb-1">Site Description</label>
                    <textarea name="site_description" rows="2"
                              class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-shadow resize-none">{{ $siteSettings['site_description'] }}</textarea>
                </div>
                <div class="mb-5">
                ...
                <button type="submit" class="w-full bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-sky-700 transition-colors shadow-sm">Save Settings</button>
            </form>
        </div>

        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-3">
            <h3 class="font-semibold text-gray-800 mb-2">New Domain Mapping</h3>
            <form action="{{ url('/admin/domain-themes') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="theme_slug" required class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none">
                ...
                <button type="submit" class="mt-2 bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-sky-700 transition-colors shadow-sm">Add Domain</button>
            </form>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($domainThemes as $dt)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-5 py-4 font-medium text-gray-800">{{ $dt->domain }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex gap-1 items-center justify-end">
                                    <form action="{{ url('/admin/domain-themes/' . $dt->id) }}" method="POST" class="inline-flex gap-1.5 items-center flex-wrap justify-end">
                                        @csrf @method('PUT')
                                        <input type="text" name="domain" value="{{ $dt->domain }}" class="border border-gray-200 rounded-lg px-2 py-1 text-xs w-28 focus:ring-2 focus:ring-sky-500 outline-none">
                                        <select name="theme_slug" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-sky-500 outline-none">
                                            @foreach($themes as $theme)
                                        ...
                                        <button type="submit" class="text-sky-600 hover:text-sky-700 text-xs font-medium">Save</button>
                                    </form>
    ...
    <div id="settingsModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-3">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="font-semibold text-gray-800">Site Settings - <span id="modalDomainName" class="text-sky-600"></span></h3>
                ...
                <form id="modalSettingsForm" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="site_description" id="modalSiteDesc" rows="2"
                                  class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-shadow resize-none"></textarea>
                    </div>
                    <div class="mb-5">
                    ...
                    <button type="submit" class="w-full bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-sky-700 transition-colors shadow-sm">Save</button>
                </form>
            </div>
        </div>
    </div>
