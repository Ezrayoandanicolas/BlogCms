...
@php
    $hl = function($text) use ($query) {
        ...
        return preg_replace($pattern, '<mark class="bg-yellow-200 text-gray-800 px-0.5 rounded">$1</mark>', e($text));
    };
@endphp
        ...
        <form action="{{ url('/search') }}" method="GET" class="mb-4">
            <div class="flex gap-2">
                <input type="text" name="q" value="{{ $query }}"
                       ...
                       class="flex-1 border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-sky-500">
                <button type="submit" class="bg-sky-600 text-white px-6 py-2 rounded-lg hover:bg-sky-700">Cari</button>
            </div>
        </form>
                    ...
                    @foreach($posts as $post)
                        <article class="bg-white rounded-lg shadow p-3">
                            <h2 class="text-sm font-semibold">
                                <a href="{{ url('/blog/' . $post->slug) }}" class="hover:text-sky-600">{!! $hl($post->title) !!}</a>
                            </h2>
