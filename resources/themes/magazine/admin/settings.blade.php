...
@section('content')
            ...
            <form action="{{ route('admin.settings.update') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="site_name" value="{{ $siteSettings['site_name'] }}" required
                           class="w-full border border-gray-200 rounded-sm px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-shadow">
                </div>
                <div class="mb-5">
                    <label class="block text-xs font-medium text-gray-500 mb-1">Site Description</label>
                    <textarea name="site_description" rows="2"
                              class="w-full border border-gray-200 rounded-sm px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-shadow resize-none">{{ $siteSettings['site_description'] }}</textarea>
                </div>
                <div class="mb-5">
                ...
                <button type="submit" class="w-full bg-red-600 text-white px-4 py-2.5 rounded-sm text-sm font-medium hover:bg-red-700 transition-colors shadow-sm">Save Settings</button>
            </form>
        </div>

        <div class="lg:col-span-2 bg-white rounded-sm shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ url('/admin/domain-themes') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="theme_slug" required class="w-full border border-gray-200 rounded-sm px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none">
                ...
                <button type="submit" class="mt-4 bg-red-600 text-white px-4 py-2.5 rounded-sm text-sm font-medium hover:bg-red-700 transition-colors shadow-sm">Add Domain</button>
            </form>
        </div>
    </div>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex gap-1 items-center justify-end">
                                    <form action="{{ url('/admin/domain-themes/' . $dt->id) }}" method="POST" class="inline-flex gap-1.5 items-center flex-wrap justify-end">
                                        @csrf @method('PUT')
                                        <input type="text" name="domain" value="{{ $dt->domain }}" class="border border-gray-200 rounded-sm px-2 py-1 text-xs w-28 focus:ring-2 focus:ring-red-500 outline-none">
                                        <select name="theme_slug" class="border border-gray-200 rounded-sm px-2 py-1 text-xs focus:ring-2 focus:ring-red-500 outline-none">
                                            @foreach($themes as $theme)
                                        ...
                                        <button type="submit" class="text-red-600 hover:text-red-700 text-xs font-medium">Save</button>
                                    </form>
                                    <button type="button" onclick="openSettingsModal({{ $dt->id }}, '{{ $dt->domain }}')"
                                            class="p-1.5 rounded-sm text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors"
                                    ...
                                    <form action="{{ url('/admin/domain-themes/' . $dt->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this domain mapping?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-1.5 rounded-sm text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors" title="Delete">
    ...
    <div id="settingsModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="bg-white rounded-sm shadow-xl w-full max-w-md p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-semibold text-gray-800">Site Settings - <span id="modalDomainName" class="text-red-600"></span></h3>
                    <button type="button" onclick="closeSettingsModal()" class="p-1 rounded-sm text-gray-400 hover:text-gray-600 hover:bg-gray-100">
                ...
                <form id="modalSettingsForm" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="site_description" id="modalSiteDesc" rows="2"
                                  class="w-full border border-gray-200 rounded-sm px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-shadow resize-none"></textarea>
                    </div>
                    <div class="mb-5">
                    ...
                    <button type="submit" class="w-full bg-red-600 text-white px-4 py-2.5 rounded-sm text-sm font-medium hover:bg-red-700 transition-colors shadow-sm">Save</button>
                </form>
            </div>
        </div>
    </div>
