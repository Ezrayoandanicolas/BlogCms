        ...
        <div class="bg-white rounded-sm shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ url('/admin/tags') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" placeholder="e.g. JavaScript" required class="w-full border border-gray-200 rounded-sm px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none transition-shadow">
                </div>
                <div class="mb-4">
                ...
                <button type="submit" class="w-full bg-red-600 text-white px-4 py-2.5 rounded-sm text-sm font-medium hover:bg-red-700 transition-colors shadow-sm">Create Tag</button>
            </form>
        </div>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <form action="{{ url('/admin/tags/' . $tag->id) }}" method="POST" class="inline-flex gap-1.5 items-center flex-wrap justify-end">
                                        @csrf @method('PUT')
                                        <input type="text" name="name" value="{{ $tag->name }}" class="border border-gray-200 rounded-sm px-2 py-1 text-xs w-20 focus:ring-2 focus:ring-red-500 outline-none">
                                        <input type="text" name="slug" value="{{ $tag->slug }}" class="border border-gray-200 rounded-sm px-2 py-1 text-xs w-20 focus:ring-2 focus:ring-red-500 outline-none" placeholder="slug">
