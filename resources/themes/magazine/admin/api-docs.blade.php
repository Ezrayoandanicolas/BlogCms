...
<div x-data="apiDocs()" class="max-w-5xl">
    ...
    @if($newToken ?? session('newToken'))
        <div class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-sm mb-6 text-sm">
        ...
        <div class="lg:col-span-2 bg-white rounded-sm shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ url('/admin/api-docs/regenerate') }}" method="POST">
                ...
                <button type="submit" onclick="return confirm('Regenerate token? Old token will stop working.')"
                        class="bg-amber-500 text-white px-4 py-2 rounded-sm text-sm font-medium hover:bg-amber-600 transition-colors shadow-sm">
                    Regenerate Token
                </button>
            </form>
        </div>
        <div class="bg-white rounded-sm shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" x-model="baseUrl" class="w-full border border-gray-200 rounded-sm px-3 py-2 text-xs focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none font-mono">
                </div>
                <div>
                    ...
                    <input type="text" x-model="bearerToken" placeholder="paste token here"
                           class="w-full border border-gray-200 rounded-sm px-3 py-2 text-xs focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none font-mono">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mb-4 flex gap-2 flex-wrap border-b border-gray-200 pb-2">
        <template x-for="g in groups" :key="g.key">
            <button @click="activeGroup = g.key"
                    :class="activeGroup === g.key ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
                    class="px-4 py-1.5 rounded-sm text-sm font-medium transition-colors"
                        ...
                        <div class="p-5 space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="bg-gray-50 rounded-sm p-3 overflow-x-auto">
                                ...
                                <div class="space-y-2">
                                            ...
                                            <div>
                                                ...
                                                <template x-if="tp.type === 'file'">
                                                    <input type="file" :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name"
                                                           class="w-full border border-gray-200 rounded-sm px-3 py-1.5 text-xs focus:ring-2 focus:ring-red-500 outline-none">
                                                </template>
                                                <template x-if="tp.type === 'textarea'">
                                                    <textarea :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name" rows="3"
                                                              class="w-full border border-gray-200 rounded-sm px-3 py-1.5 text-xs font-mono focus:ring-2 focus:ring-red-500 outline-none"
                                                ...
                                                <template x-if="tp.type !== 'file' && tp.type !== 'textarea'">
                                                    <input :type="tp.type || 'text'" :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name"
                                                           class="w-full border border-gray-200 rounded-sm px-3 py-1.5 text-xs focus:ring-2 focus:ring-red-500 outline-none font-mono"
                                    ...
                                    <div class="flex items-center gap-3 mt-3">
                                        <button @click="testEndpoint(ep)"
                                                class="bg-red-600 text-white px-4 py-1.5 rounded-sm text-xs font-medium hover:bg-red-700 transition-colors">
                                            Send Request
                                        </button>
                                    ...
                                    <div x-show="ep.response !== null" class="mt-2">
                                        ...
                                        <div class="bg-gray-900 rounded-sm p-3 overflow-x-auto max-h-64 overflow-y-auto">
...
function apiDocs() {
    ...
    return {
        ...
        methodClass(m) {
            const map = { GET: 'bg-emerald-100 text-emerald-700', POST: 'bg-red-100 text-red-700', PUT: 'bg-amber-100 text-amber-700', DELETE: 'bg-red-100 text-red-700' };
