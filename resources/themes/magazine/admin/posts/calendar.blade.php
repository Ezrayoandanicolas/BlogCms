...
@section('content')
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ url('/admin/posts/calendar?month=' . ($month == 1 ? 12 : $month - 1) . '&year=' . ($month == 1 ? $year - 1 : $year)) }}" class="inline-flex items-center gap-1 bg-gray-100 text-gray-700 px-3 py-2 rounded-sm text-sm hover:bg-gray-200 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                Previous
            </a>
            <a href="{{ url('/admin/posts/calendar') }}" class="bg-gray-100 text-gray-700 px-3 py-2 rounded-sm text-sm hover:bg-gray-200 transition">Today</a>
            <a href="{{ url('/admin/posts/calendar?month=' . ($month == 12 ? 1 : $month + 1) . '&year=' . ($month == 12 ? $year + 1 : $year)) }}" class="inline-flex items-center gap-1 bg-gray-100 text-gray-700 px-3 py-2 rounded-sm text-sm hover:bg-gray-200 transition">
                Next
            ...
            @for($day = 1; $day <= $daysInMonth; $day++)
                ...
                <div class="min-h-[100px] p-2 border-b border-r border-gray-50 {{ $isToday ? 'bg-blue-50' : ($isPast ? 'bg-white' : 'bg-amber-50/30') }}">
                    <div class="flex items-center justify-between mb-1">
                        <span class="text-sm font-semibold {{ $isToday ? 'text-red-600' : 'text-gray-700' }}">{{ $day }}</span>
                        @if($dayPosts->count() > 0)
                            <span class="text-[10px] font-medium text-white {{ $isFuture ? 'bg-amber-500' : 'bg-red-500' }} px-1.5 py-0.5 rounded-full">{{ $dayPosts->count() }}</span>
                        @endif
                    </div>
