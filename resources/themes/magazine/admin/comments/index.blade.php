...
@section('content')
    ...
    <div class="flex flex-wrap gap-2 mb-6 text-sm">
        <a href="{{ url('/admin/comments') }}" class="px-3.5 py-1.5 rounded-sm transition-colors {{ !$status ? 'bg-red-600 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">All ({{ $counts['total'] }})</a>
        <a href="{{ url('/admin/comments?status=pending') }}" class="px-3.5 py-1.5 rounded-sm transition-colors {{ $status === 'pending' ? 'bg-orange-500 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">Pending ({{ $counts['pending'] }})</a>
        <a href="{{ url('/admin/comments?status=approved') }}" class="px-3.5 py-1.5 rounded-sm transition-colors {{ $status === 'approved' ? 'bg-emerald-600 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">Approved ({{ $counts['approved'] }})</a>
        <a href="{{ url('/admin/comments?status=rejected') }}" class="px-3.5 py-1.5 rounded-sm transition-colors {{ $status === 'rejected' ? 'bg-red-500 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">Rejected ({{ $counts['rejected'] }})</a>
        <a href="{{ url('/admin/comments?status=spam') }}" class="px-3.5 py-1.5 rounded-sm transition-colors {{ $status === 'spam' ? 'bg-red-600 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">Spam ({{ $counts['spam'] }})</a>
    </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($comments as $comment)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                @if($comment->post)
                                    <a href="{{ url('/blog/' . $comment->post->slug) }}" class="text-red-600 hover:underline text-xs" target="_blank">{{ Str::limit($comment->post->title, 30) }}</a>
                                @else
                                ...
                                <div class="flex items-center justify-end gap-1">
                                    @if($comment->status !== 'approved')
                                        <form action="{{ url('/admin/comments/' . $comment->id . '/approve') }}" method="POST" class="inline">
                                            @csrf @method('PUT')
                                            <button type="submit" class="p-1.5 rounded-sm text-emerald-600 hover:bg-emerald-50 transition-colors" title="Approve">
                                    ...
                                    @if($comment->status !== 'rejected')
                                        <form action="{{ url('/admin/comments/' . $comment->id . '/reject') }}" method="POST" class="inline">
                                            @csrf @method('PUT')
                                            <button type="submit" class="p-1.5 rounded-sm text-red-500 hover:bg-red-50 transition-colors" title="Reject">
                                    ...
                                    <form action="{{ url('/admin/comments/' . $comment->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-1.5 rounded-sm text-gray-400 hover:bg-gray-100 hover:text-red-500 transition-colors" title="Delete">
