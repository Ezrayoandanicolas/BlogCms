        ...
        <form action="{{ url('/search') }}" method="GET" class="mb-8">
            <div class="flex gap-2">
                <input type="text" name="q" value="{{ $query }}"
                       ...
                       class="flex-1 border rounded-sm px-4 py-2 focus:outline-none focus:ring-2 focus:ring-red-500">
                <button type="submit" class="bg-red-600 text-white px-6 py-2 rounded-sm hover:bg-red-700">Cari</button>
            </div>
        </form>
                    ...
                    @foreach($posts as $post)
                        <article class="bg-white rounded-sm shadow p-6">
                            <h2 class="text-lg font-semibold">
                                <a href="{{ url('/blog/' . $post->slug) }}" class="hover:text-red-600">{!! $hl($post->title) !!}</a>
                            </h2>
