...
@section('breadcrumb')
    @if($post->category)
        / <a href="{{ url('/category/' . $post->category->slug) }}" class="hover:text-teal-600">{{ $post->category->name }}</a>
    @endif
...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
        <div class="flex-1 w-full self-start">
            <article class="bg-white rounded-lg shadow p-4 sm:p-6 lg:p-8">
                ...
                <div class="text-sm text-gray-500 mb-6 flex flex-wrap items-center gap-2">
                    ...
                    @if($post->category)
                        <span>&middot;</span>
                        <a href="{{ url('/category/' . $post->category->slug) }}" class="text-teal-600">{{ $post->category->name }}</a>
                    @endif
                </div>
                ...
                @if(!empty($backlinks) && count($parts) >= 2)
                    ...
                    <div style="margin:16px 0;padding:12px 16px;border-left:4px solid #2563eb;background:#f8fafc;font-size:14px">
                        <a href="{{ $url0 }}" target="_blank" rel="nofollow noopener" class="text-teal-600 hover:underline">
                            {{ $domain0 }} : {{ $title0 }}
                        </a>
                    </div>
                    ...
                    <div style="margin:16px 0;padding:12px 16px;border-left:4px solid #2563eb;background:#f8fafc;font-size:14px">
                        <a href="{{ $url1 }}" target="_blank" rel="nofollow noopener" class="text-teal-600 hover:underline">
                            {{ $domain1 }} : {{ $title1 }}
                        </a>
                    </div>
                ...
                <div class="mt-6 flex items-center gap-3 pt-4 border-t">
                    ...
                    <a href="https://www.facebook.com/sharer.php?u={{ $shareUrl }}"
                       target="_blank" rel="noopener" class="w-9 h-9 flex items-center justify-center rounded-full bg-teal-600 text-white hover:bg-teal-700 transition" title="Facebook">
                        ...
                        @foreach($relatedPosts as $rel)
                            <a href="{{ url('/blog/' . $rel->slug) }}"
                               class="bg-white rounded-lg shadow p-4 hover:shadow-md transition">
                                <h3 class="font-semibold text-sm hover:text-teal-600">{{ $rel->title }}</h3>
                ...
                <form action="{{ route('comments.store', $post->slug) }}" method="POST" class="mb-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        <input type="text" name="name" placeholder="Nama *" required
                               class="border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                               value="{{ old('name') }}">
                    ...
                    <textarea name="content" rows="4" placeholder="Tulis komentar..." required
                              class="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('content') }}</textarea>
                    @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    @error('email')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    @error('content')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    <button type="submit" class="mt-3 bg-teal-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-teal-700">
                        Kirim Komentar
                    </button>
                </form>
                        ...
                        @foreach($comments as $comment)
                            <div class="border-b pb-4">
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    <button onclick="toggleReply({{ $comment->id }})" class="text-xs text-teal-600 hover:underline ml-auto">Balas</button>
                                </div>
                                ...
                                <div id="reply-form-{{ $comment->id }}" class="hidden mt-4 ml-6">
                                    <form action="{{ route('comments.store', $post->slug) }}" method="POST">
                                        ...
                                        <div class="grid grid-cols-2 gap-3 mb-3">
                                            <input type="text" name="name" placeholder="Nama *" required
                                                   class="border rounded px-3 py-1.5 text-xs focus:ring-2 focus:ring-teal-500">
                                            <input type="email" name="email" placeholder="Email *" required
                                                   class="border rounded px-3 py-1.5 text-xs focus:ring-2 focus:ring-teal-500">
                                        </div>
                                        <textarea name="content" rows="2" placeholder="Tulis balasan..." required
                                                  class="w-full border rounded px-3 py-1.5 text-xs focus:ring-2 focus:ring-teal-500 mb-2"></textarea>
                                        <button type="submit" class="bg-teal-600 text-white px-4 py-1 rounded text-xs hover:bg-teal-700">Kirim Balasan</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
        ...
        <aside class="w-full lg:w-80 shrink-0 self-start flex flex-col gap-6">
            ...
            @include('theme::clean.widgets.categories')
            @include('theme::clean.widgets.tags')
            @include('theme::clean.widgets.recent_posts')
                        ...
                        @foreach($popularPosts as $pop)
                            <li>
                                <a href="{{ url('/blog/' . $pop->slug) }}"
                                   class="text-sm text-teal-600 hover:text-teal-800 hover:underline block">
                                    {{ $pop->title }}
                                </a>
            ...
            @endphp
            @include('theme::clean.widgets.backlinks')
        </aside>
    </div>
...
(function() {
    ...
    if (toc) {
        ...
        headings.forEach(function(h, i) {
            ...
            a.className = 'text-teal-600 hover:underline block ' + (h.tagName === 'H3' ? 'pl-3 text-xs' : '');
            li.appendChild(a);
            toc.appendChild(li);
        });
