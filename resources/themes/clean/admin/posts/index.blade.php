...
@section('content')
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <a href="{{ url('/admin/posts/create') }}" class="inline-flex items-center gap-2 bg-teal-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-teal-700 transition-colors shadow-sm whitespace-nowrap">
    ...
    <form id="bulkForm" method="POST" action="{{ url('/admin/posts/bulk') }}">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100 bg-gray-50/50 flex items-center gap-3">
                ...
                <select name="action" class="text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none">
                    <option value="">Bulk Action</option>
                    <option value="publish">Publish</option>
                    <option value="draft">Draft</option>
                    <option value="delete">Delete</option>
                </select>
                <button type="submit" class="text-sm bg-teal-600 text-white px-3 py-1.5 rounded-lg hover:bg-teal-700 transition disabled:opacity-50" id="applyBtn" disabled>Apply</button>
            </div>
                        ...
                        @foreach($posts as $post)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-3 py-4 text-center">
                                    <input type="checkbox" name="ids[]" value="{{ $post->id }}" class="post-checkbox rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                                </td>
                                <td class="px-5 py-4">
                                    <a href="{{ url('/blog/' . $post->slug) }}" class="text-teal-600 hover:text-teal-700 font-medium" target="_blank">{{ $post->title }}</a>
                                </td>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <a href="{{ url('/admin/posts/' . $post->id . '/edit') }}" class="inline-flex items-center gap-1 text-teal-600 hover:text-teal-700 text-xs font-medium mr-3">
