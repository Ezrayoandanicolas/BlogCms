        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ url('/admin/categories') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" placeholder="e.g. Technology" required class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-shadow">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="2" placeholder="Optional description" class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-shadow resize-none"></textarea>
                </div>
                <button type="submit" class="w-full bg-teal-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-teal-700 transition-colors shadow-sm">Create Category</button>
            </form>
        </div>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <form action="{{ url('/admin/categories/' . $cat->id) }}" method="POST" class="inline-flex gap-1.5 items-center flex-wrap justify-end">
                                        @csrf @method('PUT')
                                        <input type="text" name="name" value="{{ $cat->name }}" class="border border-gray-200 rounded-lg px-2 py-1 text-xs w-20 focus:ring-2 focus:ring-teal-500 outline-none">
                                        <input type="text" name="slug" value="{{ $cat->slug }}" class="border border-gray-200 rounded-lg px-2 py-1 text-xs w-20 focus:ring-2 focus:ring-teal-500 outline-none" placeholder="slug">
