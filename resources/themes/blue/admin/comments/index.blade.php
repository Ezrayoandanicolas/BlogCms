...
@section('content')
    ...
    <div class="flex flex-wrap gap-2 mb-6 text-sm">
        <a href="{{ url('/admin/comments') }}" class="px-3.5 py-1.5 rounded-lg transition-colors {{ !$status ? 'bg-cyan-600 text-white font-medium shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">All ({{ $counts['total'] }})</a>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($comments as $comment)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                @if($comment->post)
                                    <a href="{{ url('/blog/' . $comment->post->slug) }}" class="text-cyan-600 hover:underline text-xs" target="_blank">{{ Str::limit($comment->post->title, 30) }}</a>
                                @else
