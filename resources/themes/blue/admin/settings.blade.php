            ...
            <form action="{{ route('admin.settings.update') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="site_name" value="{{ $siteSettings['site_name'] }}" required
                           class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-shadow">
                </div>
                <div class="mb-5">
                    <label class="block text-xs font-medium text-gray-500 mb-1">Site Description</label>
                    <textarea name="site_description" rows="2"
                              class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-shadow resize-none">{{ $siteSettings['site_description'] }}</textarea>
                </div>
                <div class="mb-5">
                ...
                <button type="submit" class="w-full bg-cyan-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-cyan-700 transition-colors shadow-sm">Save Settings</button>
            </form>
        </div>
            ...
            <form action="{{ url('/admin/domain-themes') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="theme_slug" required class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none">
                ...
                <button type="submit" class="mt-4 bg-cyan-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-cyan-700 transition-colors shadow-sm">Add Domain</button>
            </form>
        </div>
    </div>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex gap-1 items-center justify-end">
                                    <form action="{{ url('/admin/domain-themes/' . $dt->id) }}" method="POST" class="inline-flex gap-1.5 items-center flex-wrap justify-end">
                                        @csrf @method('PUT')
                                        <input type="text" name="domain" value="{{ $dt->domain }}" class="border border-gray-200 rounded-lg px-2 py-1 text-xs w-28 focus:ring-2 focus:ring-cyan-500 outline-none">
                                        <select name="theme_slug" class="border border-gray-200 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-cyan-500 outline-none">
                                            @foreach($themes as $theme)
                                        ...
                                        <button type="submit" class="text-cyan-600 hover:text-cyan-700 text-xs font-medium">Save</button>
                                    </form>
        ...
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-semibold text-gray-800">Site Settings - <span id="modalDomainName" class="text-cyan-600"></span></h3>
                ...
                <form id="modalSettingsForm" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="site_description" id="modalSiteDesc" rows="2"
                                  class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-shadow resize-none"></textarea>
                    </div>
                    <div class="mb-5">
                    ...
                    <button type="submit" class="w-full bg-cyan-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-cyan-700 transition-colors shadow-sm">Save</button>
                </form>
            </div>
        </div>
    </div>
