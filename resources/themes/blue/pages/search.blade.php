    ...
    <div class="max-w-4xl mx-auto">
        ...
        <form action="{{ url('/search') }}" method="GET" class="mb-8">
            <div class="flex gap-2">
                ...
                <button type="submit" class="bg-cyan-600 text-white px-6 py-2 rounded-lg hover:bg-cyan-700">Cari</button>
            </div>
        </form>
                    ...
                    @foreach($posts as $post)
                        <article class="bg-white rounded-lg shadow p-6">
                            <h2 class="text-lg font-semibold">
                                <a href="{{ url('/blog/' . $post->slug) }}" class="hover:text-cyan-600">{!! $hl($post->title) !!}</a>
                            </h2>
