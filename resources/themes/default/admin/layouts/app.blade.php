            ...
            <div class="p-4 border-b border-slate-700">
                ...
                <form action="{{ url('/admin/switch-domain/' . ($currentDomainId ?? '')) }}" method="POST" id="domain-switch-form">
                    ...
                    <select name="domain_id" onchange="this.form.action=this.form.action.replace(/\/\d+$/, '/' + this.value); this.form.submit();"
                            class="w-full bg-slate-700 text-white border border-slate-600 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <main class="flex-1 flex flex-col min-w-0">
            <header class="bg-white border-b px-4 md:px-8 py-3 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3 text-sm text-gray-500">
                    ...
                    <span class="w-7 h-7 rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold">{{ strtoupper(substr(auth()->user()?->name ?? 'G', 0, 1)) }}</span>
                </div>
            </header>
