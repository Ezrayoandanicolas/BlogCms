...
@section('breadcrumb')
    @if($post->category)
        / <a href="{{ url('/category/' . $post->category->slug) }}" class="hover:text-purple-600">{{ $post->category->name }}</a>
    @endif
    / <span class="text-slate-200">{{ $post->title }}</span>
@endsection
...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
        <div class="flex-1 w-full self-start">
            <article class="bg-slate-800 rounded-lg shadow-none p-4 sm:p-6 lg:p-8">
                <h1 class="text-2xl md:text-3xl font-bold mb-4">{{ $post->title }}</h1>
                <div class="text-sm text-slate-400 mb-6 flex flex-wrap items-center gap-2">
                    ...
                    @if($post->category)
                        <span>&middot;</span>
                        <a href="{{ url('/category/' . $post->category->slug) }}" class="text-purple-600">{{ $post->category->name }}</a>
                    @endif
                </div>

                @if($post->featured_image)
                    <a href="{{ $post->featured_image }}" class="lightbox-trigger" onclick="event.preventDefault(); openLightbox(this.href)">
                        <img src="{{ $post->featured_image }}" alt="{{ $post->title }}" class="w-full rounded-lg mb-6 cursor-pointer hover:opacity-95 transition-opacity" loading="lazy">
                    </a>
                @endif
                ...
                @if(!empty($backlinks) && count($parts) >= 2)
                    ...
                    <div style="margin:16px 0;padding:12px 16px;border-left:4px solid #2563eb;background:#f8fafc;font-size:14px">
                        <a href="{{ $url0 }}" target="_blank" rel="nofollow noopener" class="text-purple-600 hover:underline">
                            {{ $domain0 }} : {{ $title0 }}
                        </a>
                    </div>
                    ...
                    <div style="margin:16px 0;padding:12px 16px;border-left:4px solid #2563eb;background:#f8fafc;font-size:14px">
                        <a href="{{ $url1 }}" target="_blank" rel="nofollow noopener" class="text-purple-600 hover:underline">
                            {{ $domain1 }} : {{ $title1 }}
                        </a>
                    </div>
                ...
                @if($post->tags->count())
                    <div class="mt-6 flex flex-wrap gap-2">
                        @foreach($post->tags as $tag)
                            <a href="{{ url('/tag/' . $tag->slug) }}" class="bg-slate-700 px-3 py-1 rounded text-sm hover:bg-gray-200">
                                #{{ $tag->name }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="mt-6 flex items-center gap-3 pt-4 border-t">
                    ...
                    <a href="https://www.facebook.com/sharer.php?u={{ $shareUrl }}"
                       target="_blank" rel="noopener" class="w-9 h-9 flex items-center justify-center rounded-full bg-purple-600 text-white hover:bg-purple-700 transition" title="Facebook">
            ...
            @if($relatedPosts->count())
                <section class="mt-8">
                    <h2 class="text-xl font-semibold mb-4">Artikel Terkait</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($relatedPosts as $rel)
                            <a href="{{ url('/blog/' . $rel->slug) }}"
                               class="bg-slate-800 rounded-lg shadow-none p-4 -none-md transition">
                                <h3 class="font-semibold text-sm hover:text-purple-600">{{ $rel->title }}</h3>
            ...
            <section class="mt-8 bg-slate-800 rounded-lg shadow-none p-4 sm:p-6 lg:p-8">
                <h2 class="text-xl font-semibold mb-6">Komentar ({{ $comments->count() }})</h2>
                ...
                <form action="{{ route('comments.store', $post->slug) }}" method="POST" class="mb-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        <input type="text" name="name" placeholder="Nama *" required
                               class="border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                               value="{{ old('name') }}">
                    ...
                    <textarea name="content" rows="4" placeholder="Tulis komentar..." required
                              class="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('content') }}</textarea>
                    @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    @error('email')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    @error('content')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    <button type="submit" class="mt-3 bg-purple-600 text-white px-6 py-2 rounded-lg text-sm hover:bg-purple-700">
                        Kirim Komentar
                    </button>
                </form>
                        ...
                        @foreach($comments as $comment)
                            <div class="border-b pb-4">
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    <button onclick="toggleReply({{ $comment->id }})" class="text-xs text-purple-600 hover:underline ml-auto">Balas</button>
                                </div>
                                <p class="text-sm text-slate-200">{{ $comment->content }}</p>

                                @if($comment->replies->count())
                                    <div class="ml-6 mt-3 space-y-3 border-l-2 border-slate-700 pl-4">
                                        @foreach($comment->replies as $reply)
                                            <div>
                                                ...
                                                <p class="text-sm text-slate-200">{{ $reply->content }}</p>
                                            </div>
                                        @endforeach
                                    </div>
                                @endif
                                ...
                                <div id="reply-form-{{ $comment->id }}" class="hidden mt-4 ml-6">
                                    <form action="{{ route('comments.store', $post->slug) }}" method="POST">
                                        ...
                                        <div class="grid grid-cols-2 gap-3 mb-3">
                                            <input type="text" name="name" placeholder="Nama *" required
                                                   class="border rounded px-3 py-1.5 text-xs focus:ring-2 focus:ring-purple-500">
                                            <input type="email" name="email" placeholder="Email *" required
                                                   class="border rounded px-3 py-1.5 text-xs focus:ring-2 focus:ring-purple-500">
                                        </div>
                                        <textarea name="content" rows="2" placeholder="Tulis balasan..." required
                                                  class="w-full border rounded px-3 py-1.5 text-xs focus:ring-2 focus:ring-purple-500 mb-2"></textarea>
                                        <button type="submit" class="bg-purple-600 text-white px-4 py-1 rounded text-xs hover:bg-purple-700">Kirim Balasan</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
                ...
                @else
                    <p class="text-sm text-slate-400">Belum ada komentar. Jadilah yang pertama!</p>
                @endif
            </section>
        </div>

        <aside class="w-full lg:w-80 shrink-0 self-start flex flex-col gap-6">
            <div class="bg-slate-800 rounded-lg shadow-none p-6" id="toc-wrapper">
                <h3 class="text-lg font-semibold mb-4 pb-2 border-b">Daftar Isi</h3>
            ...
            @if(!empty($popularPosts) && $popularPosts->count())
                <div class="bg-slate-800 rounded-lg shadow-none p-6">
                    <h3 class="text-lg font-semibold mb-4 pb-2 border-b">Populer</h3>
                    <ul class="space-y-3">
                        @foreach($popularPosts as $pop)
                            <li>
                                <a href="{{ url('/blog/' . $pop->slug) }}"
                                   class="text-sm text-purple-600 hover:text-purple-800 hover:underline block">
                                    {{ $pop->title }}
                                </a>
                                <span class="text-xs text-slate-500">{{ $pop->published_at?->diffForHumans() }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
...
(function() {
    ...
    if (toc) {
        ...
        headings.forEach(function(h, i) {
            ...
            a.className = 'text-purple-600 hover:underline block ' + (h.tagName === 'H3' ? 'pl-3 text-xs' : '');
            li.appendChild(a);
            toc.appendChild(li);
        });
