...
<div x-data="apiDocs()" class="max-w-5xl">
    <h1 class="text-xl md:text-2xl font-bold text-slate-100 mb-1">API Documentation</h1>
    <p class="text-sm text-slate-400 mb-6">Test and explore the API endpoints</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div class="lg:col-span-2 bg-slate-800 rounded-2xl shadow-none-sm border border-slate-700 p-6">
            <h3 class="font-semibold text-slate-100 mb-3">API Token</h3>
            <p class="text-xs text-slate-400 mb-3">Used by your AI generator script to authenticate API requests.</p>
            <form action="{{ url('/admin/api-docs/regenerate') }}" method="POST">
                ...
                <button type="submit" onclick="return confirm('Regenerate token? Old token will stop working.')"
                        class="bg-amber-500 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-amber-600 transition-colors shadow-none-sm">
                    Regenerate Token
                </button>
            </form>
        </div>
        <div class="bg-slate-800 rounded-2xl shadow-none-sm border border-slate-700 p-6">
            <h3 class="font-semibold text-slate-100 mb-3">API Tester</h3>
            <div class="space-y-3">
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1">Base URL</label>
                    <input type="text" x-model="baseUrl" class="w-full border border-slate-700 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none font-mono">
                </div>
                <div>
                    ...
                    <input type="text" x-model="bearerToken" placeholder="paste token here"
                           class="w-full border border-slate-700 rounded-xl px-3 py-2 text-xs focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none font-mono">
                </div>
            </div>
        </div>
    </div>

    <div class="mb-4 flex gap-2 flex-wrap border-b border-slate-700 pb-2">
        <template x-for="g in groups" :key="g.key">
            <button @click="activeGroup = g.key"
                    :class="activeGroup === g.key ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-gray-200'"
        ...
        <div x-show="activeGroup === g.key" class="space-y-4">
            <template x-for="ep in g.endpoints" :key="ep.method + ep.path">
                <div class="bg-slate-800 rounded-2xl shadow-none-sm border border-slate-700 overflow-hidden">
                    <div class="px-5 py-3 flex items-center gap-3 cursor-pointer hover:bg-slate-900"
                        ...
                        <code class="text-sm font-mono text-slate-100 truncate" x-text="ep.path"></code>
                        <span class="text-xs text-slate-500 ml-auto shrink-0" x-text="ep.desc"></span>
                    ...
                    <div x-show="ep.open" class="border-t border-slate-700">
                        <div class="p-5 space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Request</h4>
                                    <div class="bg-slate-900 rounded-xl p-3 overflow-x-auto">
                                        <pre class="text-xs font-mono text-slate-200 whitespace-pre-wrap" x-text="ep.requestExample || '—'"></pre>
                                    </div>
                                </div>
                                <div>
                                    <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Response</h4>
                                    <div class="bg-slate-900 rounded-xl p-3 overflow-x-auto">
                                        <pre class="text-xs font-mono text-slate-200 whitespace-pre-wrap" x-text="ep.responseExample || '—'"></pre>
                                    </div>
                                </div>
                            </div>
                            <template x-if="ep.params">
                                <div>
                                    <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Parameters</h4>
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-xs">
                                            <thead><tr class="border-b border-slate-700"><th class="text-left py-1.5 px-2 text-slate-400 font-medium">Name</th><th class="text-left py-1.5 px-2 text-slate-400 font-medium">Type</th><th class="text-left py-1.5 px-2 text-slate-400 font-medium">Required</th><th class="text-left py-1.5 px-2 text-slate-400 font-medium">Description</th></tr></thead>
                                            <tbody>
                                                <template x-for="p in ep.params" :key="p.name">
                                                    <tr class="border-b border-slate-800"><td class="py-1.5 px-2 font-mono" x-text="p.name"></td><td class="py-1.5 px-2 text-slate-300" x-text="p.type"></td><td class="py-1.5 px-2" x-text="p.required ? 'Yes' : 'No'"></td><td class="py-1.5 px-2 text-slate-300" x-text="p.desc"></td></tr>
                                                </template>
                                            </tbody>
                                        </table>
                                    </div>
                            ...
                            <div>
                                <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Test</h4>
                                <div class="space-y-2">
                                    <template x-if="ep.testParams">
                                        <template x-for="tp in ep.testParams" :key="tp.name">
                                            <div>
                                                <label class="block text-xs text-slate-400 mb-0.5" x-text="tp.name + (tp.required ? ' *' : '')"></label>
                                                <template x-if="tp.type === 'file'">
                                                    <input type="file" :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name"
                                                           class="w-full border border-slate-700 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-purple-500 outline-none">
                                                </template>
                                                <template x-if="tp.type === 'textarea'">
                                                    <textarea :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name" rows="3"
                                                              class="w-full border border-slate-700 rounded-lg px-3 py-1.5 text-xs font-mono focus:ring-2 focus:ring-purple-500 outline-none"
                                                ...
                                                <template x-if="tp.type !== 'file' && tp.type !== 'textarea'">
                                                    <input :type="tp.type || 'text'" :id="'ip-' + ep.method + '-' + ep.slug + '-' + tp.name"
                                                           class="w-full border border-slate-700 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-purple-500 outline-none font-mono"
                                    ...
                                    <div class="flex items-center gap-3 mt-3">
                                        <button @click="testEndpoint(ep)"
                                                class="bg-purple-600 text-white px-4 py-1.5 rounded-lg text-xs font-medium hover:bg-purple-700 transition-colors">
                                            Send Request
                                        </button>
                                        <span x-show="ep.loading" class="text-xs text-slate-500">Sending...</span>
                                    </div>
                                    <div x-show="ep.response !== null" class="mt-2">
                                        <div class="flex items-center gap-2 mb-1">
                                            ...
                                            <span class="text-xs text-slate-500" x-text="ep.responseTime ? ep.responseTime + 'ms' : ''"></span>
                                        </div>
...
function apiDocs() {
    ...
    return {
        ...
        methodClass(m) {
            const map = { GET: 'bg-emerald-100 text-emerald-700', POST: 'bg-purple-100 text-purple-700', PUT: 'bg-amber-100 text-amber-700', DELETE: 'bg-red-100 text-red-700' };
