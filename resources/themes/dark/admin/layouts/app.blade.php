...
<body class="bg-slate-900 text-slate-100 antialiased">
    <div class="min-h-screen flex flex-col lg:flex-row">
            ...
            <div class="p-4 border-b border-slate-700">
                ...
                <form action="{{ url('/admin/switch-domain/' . ($currentDomainId ?? '')) }}" method="POST" id="domain-switch-form">
                    ...
                    <select name="domain_id" onchange="this.form.action=this.form.action.replace(/\/\d+$/, '/' + this.value); this.form.submit();"
                            class="w-full bg-slate-700 text-white border border-slate-600 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
        ...
        <main class="flex-1 flex flex-col min-w-0">
            <header class="bg-slate-800 border-b px-4 md:px-8 py-3 flex items-center justify-between">
                <h1 class="text-lg font-semibold text-slate-100">@yield('title', 'Dashboard')</h1>
                <div class="flex items-center gap-3 text-sm text-slate-400">
                    <span class="hidden sm:inline">{{ auth()->user()?->name ?? 'Guest' }}</span>
                    <span class="w-7 h-7 rounded-full bg-purple-600 text-white flex items-center justify-center text-xs font-bold">{{ strtoupper(substr(auth()->user()?->name ?? 'G', 0, 1)) }}</span>
                </div>
            </header>
